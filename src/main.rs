...
impl CPUTopology {
    fn new() -> Self {
        ...
        for core_id in 0.. {
            let topology_path = format!("{}/cpu{}/topology/thread_siblings_list", cpu_path, core_id);
        ...
        CPUTopology { physical_cores: pairs }
    }
...
impl CoreManager {
            ...
            for entry in entries.flatten() {
                ...
                if name_str.starts_with("cpu") {
                    if let Ok(num) = name_str.trim_start_matches("cpu").parse::<usize>() {
                        ...
                        let topology_path = format!("{}/cpu{}/topology/thread_siblings_list", cpu_path, num);
    ...
    fn get_optimal_core_count(&mut self, on_battery: bool) -> usize {
        ...
        let active_cpus: Vec<_> = self.sys.cpus()
            .iter()
            .enumerate()
            .filter(|(i, _)| {
                if *i == 0 { return true; } // CPU0 is always considered active
        ...
        let avg_load: f32 = active_cpus.iter()
            .map(|(_, cpu)| cpu.cpu_usage())
            .sum::<f32>() / active_count as f32;

        debug!("Current average CPU load across {} active cores: {:.2}%", active_count, avg_load);
    ...
    fn manage_cpu_cores(&mut self, target_cores: usize) -> Result<(), Box<dyn Error>> {
        ...
        for core_num in available_cores.iter().skip(1) { // Skip CPU0
        ...
        info!("Core count adjusted to: {} (Enabled cores: {:?})", self.current_cores, cores_to_enable);
        Ok(())
    }
}
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    println!("Found {} CPU cores: {:?}", available_cores.len(), available_cores);
    ...
    while running.load(Ordering::SeqCst) {
        ...
        thread::sleep(Duration::from_secs(core_manager.settings.check_interval_sec));
    }
    ...
    for core_num in available_cores.iter().skip(1) {
        ...
        match fs::read_to_string(&online_path) {
            Ok(state) => {
                if state.trim() != "1" {
                    warn!("Core {} still appears to be offline after shutdown", core_num);
                }
            }
